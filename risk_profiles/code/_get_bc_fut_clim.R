...
#   gcm: GCM model
#   period: future period of interes
...
get_bc_fut_clim <- function(iso = iso,
  ...
  if(!file.exists(outfile)){
    ...
    his_bc  <<- paste0(root,"/1.Data/future_data/",gcm,"/",iso,"/bias_corrected/1995-2014/",iso,".fst")
    fut_gcm <<- paste0(root,"/1.Data/future_data/",gcm,"/",iso,"/downscale/",period,"/",iso,".fst")
    outfile <<- paste0(root,"/1.Data/future_data/",gcm,"/",iso,"/bias_corrected/",period,"/",iso,".fst")
    ...
    BC_Qmap(his_obs = his_obs,
            ...
            period  = period,
            ncores  = ncores)
    cat('Future bias-corrected climate data successfully calculated.\n')
